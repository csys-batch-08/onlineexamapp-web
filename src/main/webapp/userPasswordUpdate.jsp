...
<body>
	...
	<div class="container">
		...
		<form action="changepassword" onsubmit="return match()" method="post">
			<label for="phone_number">Enter your phone number:</label><br> <input
				type="text" pattern="[6-9]{1}[0-9]{9}" name="phone_number"
				id="phone_number" required minlength="10" maxlength="10"><br>
			<br> <label for="npassword">Enter your new Password:</label><br>
			<input type="password"
			...
			<br> <label for="cpassword">Confirm new password:</label><br>
			<input type="password"
				pattern="^(?=.*[0-9])(?=.*[a-z])(?=.*[A-Z])(?=.*[!@#&?/*$^]).{8,16}$"
				title="confirm password must be same as password" name="cpassword"
				id="cpassword" required minlength="8" maxlength="16"><br>
			<br>
			<button type="submit" class="buttons button2">Change
				Password</button>
