...
<body>
	...
	<a href="filterByDifficulty.jsp"><h4
			style="float: right; margin-top: 25px; margin-right: 15px; font-size: x-large; color: black;">
			<u><strong>Back</strong></u>
		</h4></a>
	<h2>
		<u>Exams</u>
	</h2>
	...
	<table style="width: 90%; margin-left: 60px; font-size: large;">
		...
		<c:forEach items="${easyexams}" var="easy">
			<tr>
				<c:set var="duration" value="${easy.durationMinutes}"
					scope="session" />
				<td>${count}</td>
				<td>${easy.examId}</td>
				<td>${easy.examName}</td>
				...
				<c:if test="${easy.examName=='JAVA'}">
					<td><a
						href="JavaExamEasy?examid=${easy.examId}&examName=${easy.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${easy.examName=='HTML'}">
					<td><a
						href="HtmlExamEasy?examid=${easy.examId}&examName=${easy.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${easy.examName=='CSS'}">
					<td><a
						href="CssExamEasy?examid=${easy.examId}&examName=${easy.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${easy.examName=='PYTHON'}">
					<td><a
						href="PythonExamEasy?examid=${easy.examId}&examName=${easy.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
			</tr>
