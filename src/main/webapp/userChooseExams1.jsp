...
<body>
	...
	<table style="width: 90%; margin-left: 60px; font-size: large;">
		...
		<c:forEach items="${hardexams}" var="hard">
			<tr>
				<c:set var="duration" value="${hard.durationMinutes}"
					scope="session" />
				<td>${count}</td>
				<td>${hard.examId}</td>
				<td>${hard.examName}</td>
				...
				<c:if test="${hard.examName=='JAVA'}">
					<td><a
						href="JavaExamHard?examid=${hard.examId}&examName=${hard.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${hard.examName=='HTML'}">
					<td><a
						href="HtmlExamHard?examid=${hard.examId}&examName=${hard.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${hard.examName=='CSS'}">
					<td><a
						href="CssExamHard?examid=${hard.examId}&examName=${hard.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
				<c:if test="${hard.examName=='PYTHON'}">
					<td><a
						href="PythonExamHard?examid=${hard.examId}&examName=${hard.examName}"><button
								type="submit" class="button examButton">Take Exam</button></a></td>
				</c:if>
			</tr>
